...
public class KoniecMenu {
    private final BlokTextu pocetZvysnychBalonikov; // Text s počtom zostávajúcich zombikov
    ...
    public KoniecMenu(ArrayList<Zombik> baloniky, String cesta) {
        // Obrázok pre obrazovku konca hry
        Obrazok prehraMenu = new Obrazok(cesta, 80, -50); // Inicializácia obrázku pre obrazovku konca hry
        prehraMenu.zobraz(); // Zobrazenie obrázku

        // Vytvorenie textu s počtom zostávajúcich zombikov
        this.pocetZvysnychBalonikov = new BlokTextu("Stačilo ti už zabiť iba\n" + "       " + baloniky.size() + " balónikov.", 200, 600);
